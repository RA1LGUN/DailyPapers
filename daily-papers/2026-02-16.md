...
**形式化描述**：设 $\pi_\theta(a|s)$ 为策略，目标是最大化 $\mathbb{E}_{\tau \sim \pi_\theta}[R(\tau)] + \alpha H(\pi_\theta)$，其中 $H(\pi_\theta) = -\mathbb{E}_{a \sim \pi_\theta(\cdot|s)}[\log \pi_\theta(a|s)]$。传统方法需要计算 $\log \pi_\theta(a|s)$，但扩散/流匹配模型无法直接提供。
